...
if (!User::logged_in()) {
	...
	if (!$part) //if nothing submitted yet
	{
		...
		$plan = preg_replace("/\[\<a\shref=\"[^\"]*?\"\sclass=\"planlove\"\>(.*?)\<\/a\>\]/s", "[\\1]", $plan);
		$plan = preg_replace("/\[\<a\shref=\"[^\"]*?\"\sclass=\"boardlink\"\>(.*?)\<\/a\>\]/s", "[\\1]", $plan);
		//change plan links into regular form
		$plan = preg_replace("/\[\<a\shref=\"[^\"]*?\"\sclass=\"boardlink\"\>#(.*?)\<\/a\>\]/s", "[\\1]", $plan);
		//strip out CSS info from regular links on plans
		$plan = preg_replace("/\<a\shref=\"([^\"]*?)\"\sclass=\"onplan\"\>(.*?)\<\/a\>/s", "[\\1\|\\2]", $plan);
