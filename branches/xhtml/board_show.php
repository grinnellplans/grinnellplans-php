...
//elseallowed to edit
{
	...
	if (floor($totalthreads[0] / $threadsperpage) > $pagenumber) {
		echo "<a href=\"board_show.php?pagenumber=" . floor($totalthreads[0] / $threadsperpage) . "\">&gt;&gt;</a></center>";
	} else {
		echo "&gt;&gt;</center>";
	}
	...
	%l:%i %p'), COUNT(*) From 
	mainboard,subboard where subboard.threadid=mainboard.threadid and mainboard.threadid not in (645) GROUP BY threadid ORDER BY lastupdated DESC LIMIT " . $rowoffset . "," . $threadsperpage);
	$colorswitch = 0;
	while ($new_row = mysql_fetch_row($my_result)) {
		...
		echo "<td><a href=\"board_messages.php?threadid=" . $new_row[0] . "\">" . stripslashes($new_row[1]) . "</a></td><td>" . $new_row[2] . "</td><td><center>" . $new_row[3] . "</center></td></tr>";
	}
	echo "</table>";
