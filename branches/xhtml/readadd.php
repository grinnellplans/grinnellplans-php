  ...
  else
    {
      if ($searchname)//if there is no user number given, but there is a name, but not an exact one of a user, try to search for similar ones
	    ...
	    if ($auth)//if is valid user, begin user display
            ...
            else //otherwise is a guest, so beging guest display
              {gdisp_begin($dbh);}
...
the term in them.";
            ...
            else //else there are matches, so give results
              {
                ...
                while ($partial_list[$o][0])//loop through the array of usernames that contain string, printing each one nicely.
                  {
                    echo "<li><a href=\"read.php?searchnum=" . 
            ...
            if ($auth)
              ...
              {gdisp_end();}//end guest user display
            mysql_close($dbh);
            exit();}//end page processing, since we couldn't determine user number
      else //user has submitted form, but with no user number or username
        {
          if ($auth)
            {mdisp_begin($dbh,$idcookie,$HTTP_HOST .
                         $REQUEST_URI,$myprivl);}//begin valid user display
          else
            {gdisp_begin($dbh);}//
...
{echo "Could not retrieve plan.";
 $searchnum = $idcookie;}//and set the searchnum to the persons own id, so that person does not get option to add non-existant user/plan to their autoread list
...
owner = '$idcookie' and interest = '$searchnum'");}//remove plan from users autoread list
          else
            {
...
WHERE owner = '$idcookie' AND interest = '$searchnum'");//otherwise person just wants to change which tier plan is on, so change it
  ...
  else //if person is not on users autoread list already
    {
          ...
          if (!$privlevel == 0)//make sure that person has set an actual tier level to set to, otherwise don't do anything since they don't want the plan on their autoread list.
            {
              ...
              echo "<table><tr><td><center><p class=\"sub2\">User " . $planinfo[0][0] .
                " added to autoread list with priority level of " . $privlevel .
                ".</p></center></td></tr></table>";//inform user of the change
            }
        }
    }
  mdisp_end($dbh,$idcookie,$HTTP_HOST . $REQUEST_URI,$myprivl);//end valid user display
