...
{mdisp_begin($dbh,$idcookie,$HTTP_HOST .
        $REQUEST_URI,$myprivl);}//begin user display
else
{gdisp_begin($dbh);}//otherwise begin guest display
...
else//otherwise perform the search
{
	...
	else//otherwise, go on with the search
	{
		if ($planlove)
		{
			if (!$thesearchnum = get_item($dbh, "userid", "accounts", "username",
			$mysearch))
	...
	/*
			...
			*/
			...
			echo "<li>[<a href=\"read.php?searchname=" .
			...
			for ($num=0; $num<count($start_array);$num++)
			{
				...
				if($endof === false or $endof > $end_array[$num] + 20){
					...
					if ((substr($new_row[1], $endof-2, 3) == "<br" && $i=3)
							|| (substr($new_row[1], $endof-1, 2) == "<b" && $i=2)
							|| (substr($new_row[1], $endof, 1) == "<" && $i=1))
					{
						$endof-=$i;
					}
				}
...
if ($auth)
{mdisp_end($dbh,$idcookie,$HTTP_HOST .
        $REQUEST_URI,$myprivl);}
else
{gdisp_end();}
