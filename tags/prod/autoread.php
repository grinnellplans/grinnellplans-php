...
else //allowed to edit
{
	...
	while ($arlist[$o][0])
	{
		...
		while($i<123) //do while before z
		{
			...
			{echo " <a href= \"autoread.php?myprivl=" . $myprivl . 
			"&letternum=" . $i . "\">" . chr($i) . "</a> ";}
		...
		while ($arraylist[$j][0])//do while there are names to display
		{
			...
			{echo "";}
			else //if name isn't the user's name, continue loop to display form
			{
				$mypriority[0] ="";
				$mypriority[1] ="";
				$mypriority[2] ="";
				...
				{$mypriority[$autolist[$arraylist[$j][0]][1]] = " checked";}//if the current name is on the person's autoread list, set the array so that the value attributed with the index number of the priority level will be the string "checked" to show what is currently set.
				else {$mypriority[0] = " checked";}
		...
		//pass on other info
		echo "<input type=\"hidden\" name=\"set_autoreadlist\" value=\"" . 
		$idcookie
		. "\">";
