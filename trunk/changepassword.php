	...
	if ($changed=='pass') {
		if (!(strstr($mypassword, "\"") or strstr($mypassword, "\'"))){
			if (strlen($mypassword)>3){
				...
				set_item($dbh, "accounts", "password", $crpassword,
						"userid", $idcookie);//set the password
				echo "Password changed to <b>" . $mypassword . "</b>.";//confirm changed password
			} else {echo "Could not change password. Your password must be 4 or more characters.";}
		} else {echo "Illegal character in password. Please do not use \" or '.";}
	} 
...
<a href="http://www.grinnellplans.com/read.php?searchname=<?=$username?>&amp;guest-pass=<?=$real_pass?>">
http://www.grinnellplans.com/read.php?searchname=<?=$username?>&amp;guest-pass=<?=$real_pass?>
</a> <br />
<? } else { ?>
...
<? } ?>
		...
		<form method="POST" action="">
		<input type="text" name="guest_password" value="<?=$real_pass?>">
		<input type="hidden" name="myprivl" value="<?php echo $myprivl; ?>">
