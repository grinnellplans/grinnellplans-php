...
else //allowed to edit
{
  ...
  if ($part)//if form has been submitted
    { 
      set_item($dbh, "display", "interface", $interface,
               "userid", $idcookie);//set which interface they selected
 ...
 //Let user know the interface has been set
   ...
   <center><h2>Interface Options:</h2>  
      <table><tr><Td>Interface Set</td></tr></table></center><?
      
      
      }
  else //if not submitted, give form
    {
      mdisp_begin($dbh,$idcookie,$HTTP_HOST . $REQUEST_URI,$myprivl); //begin valid user display
...
interface");//get the current interfaces and descriptions
      ...
      $intcheck[get_item($dbh,"interface","display",userid, 
                         $idcookie)] = " checked"; //get user's current selection, and set it to the index of an array and put value to checked
...
value=\"";
          echo $myinterfaces[$o][0] . "\"" . $intcheck[$myinterfaces[$o][0]] . 
            "><td>" . $myinterfaces[$o][1] . "</td></tr>"; //show each option
          $o++;
        }
