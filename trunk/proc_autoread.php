          ...
          if ($prosplist[$arlist[$o][0]])
            {
              if ($arlist[$o][1] == $prosplist[$arlist[$o][0]]) //if priorities match
                {$remain[$arlist[$o][0]] = 1;}//set to neither delete, nor add
              else //if priorities don't match
                {$ridof[] = $arlist[$o][0];}
              
            }
          
          else //if not on prospective list
            {$ridof[] = $arlist[$o][0];
            }
  ...
  while($i<123) //while before z
    {
      if ($i==$letternum)//if we've hit the desire letter
        ...
        {echo " <a href= \"autoread.php?myprivl=" . $myprivl . "&letternum="
           . $i . "\">" . chr($i) . "</a> ";}
      $i++;
    }
