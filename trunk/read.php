...
'%a %M %D %Y, %l:%i %p'),plan,webview","accounts","userid", $searchnum))//get all of persons plan info
{
	...
	$searchnum = $idcookie;
		...
		if (!$idcookie && $planinfo[0][5] !=1 && !$guest_auth)
		{
			...
			echo "<table><tr><td><p class=\"main\">Username: </p></td><td><b>" .
				$planinfo[0][0] . "</b></td></tr></table>";
			...
			echo "<table><tr><td><p class=\"main4\">Name:</p></td><td><u>" .
				$planinfo[0][1] . "</u></td></tr></table>";
	...
	if (strlen($mysearch)<3){
		...
		else//otherwise, go on with the search
		{
			...
			while ($new_row = mysql_fetch_row($my_result)) {
				...
				$matchcount = preg_match_all("/(" . preg_quote($mysearch,"/") .  ")/si", $new_row[1],
						$matcharray);

				$new_row[1] = preg_replace("/("  .preg_quote($mysearch,"/") . ")/si", "<b>\\1</b>",
						$new_row[1]);
